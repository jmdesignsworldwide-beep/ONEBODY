...
create policy projects_select on public.projects for select
  using ( public.project_is_public(status) or private.is_admin() );
...
create policy donations_select_own on public.donations for select to authenticated
  using ( donor_id = (select auth.uid()) or private.is_admin() );
...
create policy subs_update on public.subscriptions for update to authenticated
  using ( donor_id = (select auth.uid()) or private.is_editor() )
  with check ( donor_id = (select auth.uid()) or private.is_editor() );
...
-- Sin política de INSERT: la escritura ocurre sólo vía `log_audit`
-- (SECURITY DEFINER, dueño `postgres` con BYPASSRLS) o `service_role`
-- (BYPASSRLS). anon/authenticated no tienen ni GRANT ni política → no pueden
-- insertar. Se evita una política "always true" (limpio ante Security Advisor).
